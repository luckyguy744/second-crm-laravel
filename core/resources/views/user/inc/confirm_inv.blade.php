  ...
  <div class="row wd_row_pad" >
    ...
    <div class="col-md-4 card pop_invest_col" align="center">      
      <div class="card-header" style="">
        <h3><b>{{ __('messages.init_invstm') }}</b></h3>
        <h5>{{ __('messages.wlt_bal') }} <b>{{$settings->currency}} <span id="WalletBal"></span></b></h5>         
        <hr>
      </div>
      <div class="pop_msg_contnt">              
        <p align="center" class="color_blue_b">
            {{ __('messages.abt_to_invest') }} <b><span id="pack_inv"></span></b> {{ __('messages.pkg_ivt') }} {{ __('messages.pkg_wking') }}  <b><span id="intr"></span></b>% {{ __('messages.tt_int') }}                  
        </p>
        <form id="userpackinv" action="/user/invest/packages" method="post">
            <div class="form-group" align="left">
              <div class="pop_form_min_max" align="center">
                <b>{{ __('messages.min_cap_ivt') }} {{$settings->currency}} <span id="min"></span></b> | 
                <b>{{ __('messages.max_cap_ivt') }} {{$settings->currency}} <span id="max"></span></b>                      
              </div> 
              <br>                   
              <label>{{ __('messages.amt_to_ivt') }}</label>
            ...
            <div class="form-group">
                ...
                <span style="">            
                  <a id="popMsg_close_user" href="javascript:void(0)" class="btn btn-danger">{{ __('messages.cncl') }}</a>        
                </span>
                <br><br>
            </div>
        </form>
